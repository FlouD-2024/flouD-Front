...
export default function UnFriend({}: Props) {
  const [isModalOpened, setIsModalOpened] = useRecoilState(friendModalOpenAtom);
  const [name, setName] = useRecoilState(friendModalFriendNameAtom);
  return (
    <>
        <ModalLayout>
            <div className='fixed flex-col bg-[white] w-[450px] h-[370px] rounded-[50px] text-center pb-[25px] pt-[55px] m-auto top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2'>
                ...
                <button className='text-[20px] bg-[black] text-white w-[110px] h-[40px] rounded-[50px] mt-[30px] mr-[10px]' onClick={()=>setIsModalOpened(false)}>네</button>
