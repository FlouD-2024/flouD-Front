...
export default function MypageInfoEdit({
  nickname,
  goallist,
  introduction,
  setNickname,
  ...
  const onEditComplete = () => {
    putUserInfo({
      nickname,
      introduction,
      goalList: goallist.concat(
        addedDDay.filter((dday) => dday.content && dday.deadline)
      ),
    }).then((data) => setIsEdit(false));
  };
    ...
    <>
      <div className="bg-[white] py-[40px] px-[50px]">
        ...
        <div className="mb-[30px] flex justify-start gap-[10px]">
          ...
          <input
            className="bg-[#4C6FFF]/10 rounded-[10px] w-[300px] h-[40px] p-[5px] px-[15px]"
            value={nickname}
            onChange={(e) => {
          ...
          {isNickNameChanged && (
            <button
              className="bg-[#4C6FFF] text-white px-[10px] py-[5px] rounded-[10px]"
        ...
        <div className="mb-[30px]">
          ...
          <textarea
            className="bg-[#4C6FFF]/10 w-[100%] rounded-[10px] p-[15px] px-[15px]"
            value={introduction}
        ...
        <div>
          ...
          <div className="flex justify-start gap-[20px]">
            {goallist.map((item, i) => {
              return (
                <Dday
                  ...
                  goalId={item.goal_id}
                  isEdit={true}
                  onDeleteClick={() => onDDayDeleteClick(i)}
                />
              );
            })}
            {addedDDay.map((item, i) => {
              return (
                <DdayEdit
                  ...
                  onDDayNameChange={(e) =>
                    onAddedDdayNameChange(i, e.target.value)
                  }
            ...
            {goallist.length + addedDDay.length < 3 ? (
              <DdayAdd
                isEdit={true}
                onDDayAddClick={() =>
                  setAddedDDay((org) => [...org, { content: "", deadline: "" }])
                }
              />
            ) : (
              <></>
          ...
          <div className="flex">
            <button
              disabled={
                isNickNameChanged && !(isNickNameCheck && !isNickNameDuplicated)
              }
              className={`bg-[${
                isNickNameChanged && !(isNickNameCheck && !isNickNameDuplicated)
                  ? "#9C9C9C"
                  : "#4C6FFF"
              }] text-white px-[30px] py-[5px] rounded-[10px] my-[30px] mx-[auto]`}
