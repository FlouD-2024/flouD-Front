...
import { getRetroDetailType, getRetroListType, postRetroDetailType, RetroDetailParam, RetroListParam } from "@/types/retroType";
...
export const writeRetro = async (req: WriteRetroReq) => {
  const response = await request<WriteRetroReq, postRetroDetailType, null>({
    uri: `/api/memoir`,
    method: "post",
    data: req,
  })
